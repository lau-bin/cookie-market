...
export const PopUp = ({
  ...
  const bid =
    Object.keys(popUp.token.bids).length > 0
      ? formatNearAmount(
          Object.entries(popUp.token.bids).map(([ft_token_id, ft_token_bids]) =>
  ...
  const handleOfferClick = (e) => {
    ...
    else if (parseFloat(bid) === 0 && parseFloat(offerPrice) <= 0)
      setFeedBack("Offer must be greater than 0");
  ...
  return (
    <div
      onClick={(e) => {
        e.target.classList.contains("backgroundPopUp") && setAnimation(true);
      }}
      className={
        animation ? "backgroundPopUp exit-popup" : "backgroundPopUp show-popup"
      }
      tabIndex={-1}
    >
      {show && (
          ...
          <div className="cookieInfo">
            <div>
              <div className="frameMetaInfoe1">
                {popUp.token &&
                  Object.entries(popUp.token?.sale_conditions).map(
                    ([ft_token_id, price]) => (
                      <span
                        className="cost"
                        key={ft_token_id}
                ...
                >
                  {popUp.token?.accountId !== popUp.token?.owner_id
                    ? `${formatAccountId(popUp.token?.owner_id, 12)}`
                    : `Your NFT`}
                </span>
              </div>
              {false && (
              ...
              {accountId.length > 0 && accountId !== popUp.token.owner_id && (
                <div style={{ display: "flex", flexDirection: "column" }}>
                  ...
                  >
                    ...
                    />
                    <button
                      style={{ width: "80px", borderRadius: "0 8px 8px 0" }}
              ...
              {accountId === popUp.token.owner_id && (
                <>
                  ...
                  <button
                    ...
                    onClick={() => {
                      setShow(false);
                      handleSaleRemove(account, popUp.token.token_id);
                    }}
                  >
                    Remove From Market
                  </button>
            ...
            <div>
              {Object.keys(popUp.token.bids).length > 0 && (
                <>
                  ...
                  {Object.entries(popUp.token.bids).map(
                    ([ft_token_id, ft_token_bids]) =>
                      ft_token_bids
                        .filter(
                          (element) =>
                            element === ft_token_bids[ft_token_bids.length - 1]
                        )
                        .map(({ owner_id: bid_owner_id, price }) => (
                          <div className="offers" key={ft_token_id}>
                            {accountId !== popUp.token.owner_id && (
                              ...
                              >
                                ...
                                {token2symbol[ft_token_id]} by{" "}
                                {formatAccountId(bid_owner_id, 25)}
                              </span>
                            )}
                            {accountId === popUp.token.owner_id && (
                              <button
                                onClick={() => {
                                  ...
                                  handleAcceptOffer(
                                    account,
                                    popUp.token.token_id,
                                    ft_token_id
                                  );
                                }}
                              >
                                Accept {formatNearAmount(price, 4)} N by{" "}
                                {formatAccountId(bid_owner_id, 22)}
                              </button>
                            )}
                          </div>
                        ))
